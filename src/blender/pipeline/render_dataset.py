...
def main(argv=None):
    ...
    if os.path.isabs(args.config):
        ...
        # Accept paths relative to either the caller's working directory or this script.
        cwd_config = os.path.abspath(args.config)
        script_config = os.path.join(_SCRIPT_DIR, args.config)
        config_path = cwd_config if os.path.isfile(cwd_config) else script_config
    ...
    run_meta = {
        ...
        "pipeline":    "pipeline (fixed camera + object transform)",
    }
    if args.shard_id == 0:
    ...
    #  构建渲染操作
    # ─────────────────────────────────────────────────
    from pipeline import build_ops, RenderContext
    ...
    sys.stdout.write(f"[render] pipeline: {len(pre_render_ops)} pre-render ops, "
