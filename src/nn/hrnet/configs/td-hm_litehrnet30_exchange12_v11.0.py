...
custom_imports = dict(
    imports=[
        "nn.hrnet.custom.albumentation",
        "nn.hrnet.custom.visibility_head",
    ],
...
data_root = os.path.abspath(
    os.environ.get("HRNET_DATA_ROOT", "datasets/exchange12")
)
ann_root = os.path.abspath(
    os.environ.get("HRNET_ANN_ROOT", "annotations/exchange12")
)
load_from = os.environ.get("HRNET_LOAD_FROM", None)
...
model = dict(
    ...
    head=dict(
        type="HeatmapHeadWithVisibility",
        in_channels=40,
        out_channels=12,
...
train_pipeline = [
    ...
    dict(
        type="PixelAlbumentation",
        transforms=[
            dict(type="GaussianBlur", blur_limit=(3, 7), p=0.01),
            dict(type="GaussNoise", std_range=(0.04, 0.09), p=0.03),
            dict(type="MotionBlur", blur_limit=5, p=0.2),
            dict(type="ImageCompression", quality_range=(50, 100), p=0.4),
            dict(type="RandomShadow", num_shadows_limit=(1, 3), p=0.1),
            ...
            dict(type="Downscale", scale_range=(0.5, 0.9), p=0.1),
            dict(type="RandomFog", fog_coef_range=(0.1, 0.4), p=0.02),
            ...
            dict(type="GridDropout", ratio=0.3, unit_size_range=(10, 80), p=0.02),
            dict(type="MedianBlur", blur_limit=5, p=0.03),
            dict(type="ChannelDropout", channel_drop_range=(1, 2), fill=0, p=0.02),
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_mode=data_mode,
        ann_file=os.path.join(ann_root, "exchange12_train.json"),
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_mode=data_mode,
        ann_file=os.path.join(ann_root, "exchange12_val.json"),
...
val_evaluator = [
    dict(type="CocoMetric", ann_file=os.path.join(ann_root, "exchange12_val.json")),
