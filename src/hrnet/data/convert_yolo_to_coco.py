...
The public training path defaults to the 12-keypoint exchange target. The
converter preserves raw visibility and in-frame flags and can optionally write
preview images for inspection.
"""
...
def process_split(
    ...
    for label_path in label_files:
        ...
        for line in raw_lines:
            try:
                records.append(parse_label_line(line, total_kpts=len(FULL_KPT_NAMES)))
        ...
        instances, skipped = build_exchange12_instances(
            ...
            pillar_kpts=len(PILLAR_KPT_NAMES),
        )
...
def process_dataset_split(
    ...
    process_split(
        ...
        output_json=os.path.join(out_dir, f"exchange12_{split}.json"),
        pillar_class_id=pillar_class_id,
        exchange_class_id=exchange_class_id,
        vis_dir=vis_dir,
        vis_num=vis_num,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="maximum preview images to export for each split when --vis-dir is set",
    )
