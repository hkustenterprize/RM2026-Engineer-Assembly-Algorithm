"""
td-hm_litehrnet30_pillar_geo_vis.py
===================================
与 td-hm_litehrnet30_pillar_geo.py 完全相同，但使用 PillarHeatmapHeadWithVis
替代 PillarHeatmapHead，启用 visibility 预测分支。

Visibility 分支结构：
   backbone feats (40ch) → AdaptiveAvgPool2d(1) → Flatten → Linear(40, 5) → Sigmoid
   → per-keypoint visibility score in [0, 1]

训练时额外损失：
   loss_vis = BCE(vis_pred, binary_gt)   (v=1→0, v=2→1)

推理时：
   pred_instances.keypoints_visible ← vis_head output

使用方式：
   use_vis=False  → 行为与 PillarHeatmapHead 完全一致（零额外开销）
   use_vis=True   → 启用 vis_head 分支
...
训练命令:
   ...
   /data/datasets/zguobd/RM2026-Engineer-Host/cv/nn/hrnet/model_configs/td-hm_litehrnet30_pillar_geo_vis.py \
   --work-dir /data/datasets/zguobd/RM2026-Engineer-Host/cv/nn/hrnet/runs/litehrnet30_geo_vis_v1.0
"""
...
load_from = "/data/datasets/zguobd/RM2026-Engineer-Host/cv/nn/hrnet/runs/litehrnet30_v1.0/epoch_5.pth"
...
# ──────────────────────────────────────────────────────────────────────────────
#  Model  — 使用 PillarHeatmapHeadWithVis 替代 PillarHeatmapHead
# ──────────────────────────────────────────────────────────────────────────────
model = dict(
    ...
    head=dict(
        type="PillarHeatmapHeadWithVis",        # ← 带 vis 的版本
        use_vis=True,                            # ← 启用 visibility 预测
        vis_loss=dict(
            type="BCELoss",
            use_target_weight=False,
            use_sigmoid=True,
        ),
        vis_target_mode="binary",                # v=1→0(occluded), v=2→1(visible)
        # ── 以下参数与 PillarHeatmapHead 完全一致 ──────────────────────────────
        in_channels=40,
        out_channels=5,
...
train_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                type="GridDropout", ratio=0.3, unit_size_min=10, unit_size_max=80, p=0.2
            ),
